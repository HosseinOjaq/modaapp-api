...
public interface IRedisCache
{
    ...
    bool Exists(string key, int dbIndex = 0);

    Task<bool> ExistsAsync(string key, int dbIndex = 0, CancellationToken token = default);

    TimeSpan? TimeToLive(string key, int dbIndex = 0);

    Task<TimeSpan?> TimeToLiveAsync(string key, int dbIndex = 0, CancellationToken token = default);
}